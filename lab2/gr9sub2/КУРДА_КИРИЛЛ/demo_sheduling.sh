...
# Создаем конфигурацию для демонстрации
cat > $CONFIG_FILE << 'EOF'
{
    "workers": 4,
    "mode_heavy": {
...
# Собираем метрики в фоне
echo "Collecting CPU metrics for 10 seconds..."
...
echo "Pidstat output summary:"
echo "======================="

# Анализируем вывод pidstat
for i in {0..3}; do
...
if [ -n "${WORKER_PIDS[0]}" ] && [ -n "${WORKER_PIDS[1]}" ]; then
    ...
    # Меняем nice значения
    sudo renice -n 5 -p ${WORKER_PIDS[0]} 2>/dev/null || renice -n 5 -p ${WORKER_PIDS[0]}
    sudo renice -n 15 -p ${WORKER_PIDS[1]} 2>/dev/null || renice -n 15 -p ${WORKER_PIDS[1]}

    echo "New nice values:"
    echo "Worker 0: $(ps -o nice= -p ${WORKER_PIDS[0]})"
    echo "Worker 1: $(ps -o nice= -p ${WORKER_PIDS[1]})"

    echo "Collecting metrics after nice change..."
    pidstat -u -p ${WORKER_PIDS[0]},${WORKER_PIDS[1]} 1 5 > pidstat_after.txt

    echo "Metrics after nice change:"
