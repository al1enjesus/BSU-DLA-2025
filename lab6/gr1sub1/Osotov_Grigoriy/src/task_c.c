...
/**
 * @brief Преобразует содержимое буфера в верхний регистр.
 *
 * @param str Буфер для преобразования.
 * @param len Размер буфера.
 */
static void uppercase_transform(char *str, size_t len) {
...
static int passthrough_read(const char *path, char *buf, size_t size, off_t offset,
    ...
    // Если данные успешно прочитаны, преобразуем их в верхний регистр
    if (res > 0) {
        uppercase_transform(buf, res);
    }
...
int main(int argc, char *argv[]) {
    ...
    // FUSE получает все аргументы, но мы должны "удалить" наш первый аргумент
