...
static void update_stats(const char *op, ssize_t bytes) {
    ...
    else if (strcmp(op, "rmdir") == 0) stats.rmdirs++;
...
static int mon_getattr(const char *path, struct stat *stbuf, struct fuse_file_info *fi) {
    (void) fi;
...
static int mon_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
    (void) fi;
    if (is_stats_file(path)) {
        char stats_buf[1024];
...
static int mon_write(const char *path, const char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
    (void) fi;
    if (is_stats_file(path)) return -EACCES;
...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        fprintf(stderr, "Использование: %s <source_dir> <mount_point>\n", argv[0]);
        return 1;
    }
    ...
    fprintf(stderr, "Мониторинг %s в %s\n", base_path, argv[1]);
    fprintf(stderr, "Статистика доступна по %s%s\n", argv[1], STATS_FILE);
