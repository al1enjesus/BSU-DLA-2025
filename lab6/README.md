...
**Выполнил:** Соколов Е.А. (Номер 14, Вариант 2)  
...
```bash
# Установка FUSE и инструментов сборки
make install-deps
...
cat /tmp/lab6_source/secret.txt    # Зашифровано: "Uryyb Jbeyq"
cat /tmp/lab6_mount/secret.txt     # Расшифровано: "Hello World"
...
# Тестирование uppercase преобразования
echo "hello world" > /tmp/lab6_source/test.txt
cat /tmp/lab6_source/test.txt      # Оригинал: "hello world"
cat /tmp/lab6_mount/test.txt       # Uppercase: "HELLO WORLD"
...
│   ├── passthrough_fs.c    # Задание A
│   ├── rot13_fs.c          # Задание B
│   └── uppercase_fs.c      # Задание C
├── bin/                    # Исполняемые файлы (после сборки)
├── obj/                    # Объектные файлы (после сборки)
├── Makefile                # Система сборки
├── README.md               # Эта инструкция
├── REPORT.md               # Детальный отчет
└── test_fuse.sh            # Скрипт тестирования
```
...
# Отдельные программы
make passthrough_fs   # Собрать только passthrough FS
make rot13_fs         # Собрать только ROT13 FS
make uppercase_fs     # Собрать только uppercase FS

# Установка и проверка
make install-deps     # Установить зависимости FUSE
make check-deps       # Проверить зависимости

# Тестирование
make setup            # Создать тестовые директории
make test             # Создать тестовые файлы
make demo-rot13       # Демонстрация ROT13
make demo-uppercase   # Демонстрация uppercase

# Утилиты
make unmount          # Размонтировать FUSE FS
make clean-all        # Полная очистка (включая тест данные)
```

## Реализованные FUSE операции

Все программы поддерживают следующие файловые операции:

- `getattr()` — получение метаданных файла
...
### Задание A: Passthrough Filesystem
- Прозрачное проксирование всех операций к реальной ФС
- Логирование каждой операции в stderr с timestamp
- Поддержка всех базовых файловых операций

### Задание B: ROT13 Encryption Filesystem
- Автоматическое шифрование при записи
- Автоматическое расшифровывание при чтении
- ROT13 алгоритм (сдвиг букв на 13 позиций)
- Небуквенные символы остаются без изменений

### Задание C: Uppercase Filesystem
- Преобразование всего текста в верхний регистр при чтении
- Оригинальные файлы остаются без изменений на диске
- Запись происходит в оригинальном регистре
...
- libfuse3-dev
- gcc, make, pkg-config
- Права на монтирование FUSE файловых систем

## Отладка

Для отладки используйте флаги:
- `-f` — запуск в foreground режиме
- `-d` — включить debug вывод от libfuse
- `-s` — однопоточный режим

Пример:
```bash
./bin/passthrough_fs /tmp/lab6_source /tmp/lab6_mount -f -d
```
...
fusermount -u /tmp/lab6_mount
# или принудительно:
fusermount -uz /tmp/lab6_mount
```

## Контакты

При возникновении вопросов обращайтесь к автору или преподавателю.

---

**Лабораторная работа выполнена в соответствии с требованиями задания для варианта 2 (четные номера в группе).**
