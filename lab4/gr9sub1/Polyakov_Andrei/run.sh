...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Lab 4 Experiments - Variant 19${NC}"
...
$(TARGET): test_prog.o
	$(CC) $(CFLAGS) -o $(TARGET) test_prog.o
...
if command -v $PERF_CMD &> /dev/null; then
    $PERF_CMD stat -e cycles,instructions,context-switches,page-faults,cache-misses \
