...
void init() {
    ...
    fprintf(stderr, COLOR_BLUE "[SPY] Monitoring: gcc, make, as\n" COLOR_RESET);
}
...
ssize_t write(int fd, const void *buf, size_t count) {
    ...
    if (fd != 2) {
        if (result >= 0) {
            fprintf(stderr, COLOR_CYAN "[SPY] write(fd=%d, buf=%p, count=%zu) = %zd" COLOR_RESET "\n",
                    fd, buf, count, result);
        } else {
...
int close(int fd) {
    ...
    if (result >= 0) {
        fprintf(stderr, COLOR_MAGENTA "[SPY] close(fd=%d) = %d\n" COLOR_RESET, fd, result);
    } else {
