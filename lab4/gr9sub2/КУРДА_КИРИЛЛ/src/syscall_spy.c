// libsyscall_spy: перехват open/openat/read/write/close для сетевых программ
#define _GNU_SOURCE
#include <dlfcn.h>
#include <stdio.h>
#include <fcntl.h>
...
ssize_t write(int fd, const void *buf, size_t count) {
    ...
    // НЕ логируем запись в stderr (fd==2) чтобы избежать бесконечной рекурсии
    if (fd != 2) {
