...
# Вывод: "Hello from Vladislav module!"
...
**Скриншот dmesg:**
```
[ 7543.202064] Hello from Vladislav module!
[ 7563.779575] Goodbye from Vladislav module!
[ 7595.880383] HI!
[ 7606.806626] Goodbye from Vladislav module!
[ 7638.905728] HI!
[ 7646.130469] BYE!
```
...
Модуль ядра — это компонент операционной системы, который загружается в её ядро для расширения функциональности.
...
Если в модуле ядра обратиться к NULL-указателю — произойдёт крах системы (kernel panic) с полной остановкой ОС.

**4. Почему нельзя использовать `printf()` в модуле ядра?**
Потому что printf() — это библиотечная функция языка C из пользовательского пространства, которая недоступна в ядре.

**5. Что такое kernel panic и как его избежать?**
Kernel Panic — это критическая ошибка ядра операционной системы, которая делает её дальнейшую работу невозможной и приводит к аварийной остановке. Это механизм защиты целостности данных. Избежать можно через тщательную проверку указателей, границ массивов, возвращаемых значений и работу в VM.
...
Функция module_exit() объявляет функцию очистки, которая выполняется при выгрузке модуля ядра из памяти.
...
Нет, нельзя выгрузить модуль, если он используется системой или процессами.
...
`KERN_EMERG`, `KERN_ALERT`, `KERN_CRIT`, `KERN_WARNING`, `KERN_ERR`, `KERN_NOTICE`,`KERN_DEBUG` (0-7), `KERN_INFO`.

**12. Как посмотреть логи модуля?**
Через `dmesg`, `dmesg -w`, `/var/log/kern.log`, `journalctl -k`.

**13. Что означает "tainted kernel"?**
"Испорченное" ядро — это состояние ядра Linux, когда оно было скомпрометировано или модифицировано непроверенными компонентами, что может повлиять на стабильность и поддержку.
...
Если не освободить память в module_exit(), произойдёт утечка памяти в пространстве ядра (kernel memory leak).
...
/proc — это виртуальная файловая система в Linux, которая предоставляет интерфейс для взаимодействия с ядром и процессами в реальном времени.
...
MODULE_LICENSE() — это макрос, который указывает лицензию модуля ядра Linux.
...
Перезагрузить систему, изучить логи, исправить код, использовать VM для тестирования.
