...
Данная лабораторная работа посвящена изучению разработки модулей ядра Linux. Реализованы три модуля:
    ...
    hello_module - простой модуль с приветствием и поддержкой параметров
    proc_module - модуль с файлом /proc/my_config для чтения и записи
    sys_stats_module - модуль для отображения системной статистики через /proc/sys_stats
...
Ожидаемый результат:
    ...
    При загрузке без параметра: "Hello from Nihlebov Ivan module!"
    При загрузке с параметром: ваше сообщение
    При выгрузке: "Goodbye from Nihlebov Ivan module!"
